...
class HandMadeCoordinates(CoordinateBase):
    ...
    def __init__(
            self, chr_bounds: List[Chromosome_Info], window_size: int, file_path: str):
        super().__init__(
            chr_bounds,
            window_size,
        )
...
class RandomCoordinates(CoordinateBase):
    ...
    def __init__(
            self, chr_bounds: List[Chromosome_Info], window_size: int, seed):
        super().__init__(
            chr_bounds,
            window_size
        )
    ...
    def get_next_coord(self):
        ...
        while (not find_chr):
            ...
            if (rnd_start + self.window_size <
                    rnd_bounds.start_pos + rnd_bounds.lenght):
                # return `chromosome num`, `start position` in chromosome and
