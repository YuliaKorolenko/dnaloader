...
if __name__ == '__main__':
    ...
    chr_full = CharacteristicFullHiC("/home/ojpochemy/dnaloader/hic_r4")
    ...
    for i in range(0, 1000):
        ...
        random_window = random.randint(0, 10000)
        
        my_answer = chr_full.get_lines(random_chr, random_number_1, random_number_2, random_window)

        right_answer = chr_with_coller.get_line_d(random_chr, random_number_1, random_number_2, random_window)
        if (len(my_answer) == len(right_answer) and len(my_answer[0]) == len(right_answer[0])): 
            print("Is a square")
