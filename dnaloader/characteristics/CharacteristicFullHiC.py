...
class HICFull():
    ...
    def generate_new_file(self) -> None:
        ...
        meta_grp = file.create_group("indexes")
    ...
    def get_meta(self, start_r: int, start_c: int,
        ...
        s_r_end = math.ceil((start_r + window_size) / self.square_size)
        s_c_start = start_c // self.square_size
        s_c_end = math.ceil((start_c + window_size) / self.square_size)
...
class CharacteristicFullHiC(Characteristic):
    def __init__(self, path: str, hic_path: str = ""):
        ...
        self.hic_path = hic_path
        if (self.hic_path == ""):
            self.my_h5 = HICFull(self.path, 1_000)

    def preprocess(self):
        # preprocessing in progress
        hic_prev = HiCCollerFormat(hic_path=self.hic_path)
        my_h5 = HICFull(self.path, 1_000)
    ...
    def get_chr_lenght(self, chr: int):
        return self.my_h5.get_chr_end(chr)
    ...
    def get_lines(self, chr: int, start_0: int,
        ...
        for _, square in squares.iterrows():
            ...
            cur_pixels['c_r_1'] = cur_pixels['bin1_id'] + \
                square["square_rows"] * self.my_h5.square_size - new_start_0
            cur_pixels['c_c_1'] = cur_pixels['bin2_id'] + \
                square["square_columns"] * self.my_h5.square_size - new_start_1

            cur_pixels['c_r_2'] = cur_pixels['bin2_id'] + \
                square["square_columns"] * self.my_h5.square_size - new_start_0
            cur_pixels['c_c_2'] = cur_pixels['bin1_id'] + \
                square["square_rows"] * self.my_h5.square_size - new_start_1
